...
func TestDefaultFormatter(t *testing.T) {
	out := DefaultFormatter(time.Now(), LOG_ERR, "hostname", "tag", "content")
...
func TestUnixFormatter(t *testing.T) {
	out := UnixFormatter(time.Now(), LOG_ERR, "hostname", "tag", "content")
...
func TestRFC3164Formatter(t *testing.T) {
	out := RFC3164Formatter(time.Now(), LOG_ERR, "hostname", "tag", "content")
	expected := fmt.Sprintf("<%d>%s %s %s[%d]: %s",
...
func TestRFC5424Formatter(t *testing.T) {
	out := RFC5424Formatter(time.Now(), LOG_ERR, "hostname", "tag", "content")
