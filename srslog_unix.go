...
func (n *localConn) writeString(framer Framer, formatter Formatter, timestamp time.Time, p Priority, hostname, tag, msg string) error {
	if framer == nil {
	...
	_, err := n.conn.Write([]byte(framer(formatter(timestamp, p, hostname, tag, msg))))
	return err
}
