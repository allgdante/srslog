...
type Formatter func(timestamp time.Time, p Priority, hostname, tag, content string) string
...
func DefaultFormatter(timestamp time.Time, p Priority, hostname, tag, content string) string {
	msg := fmt.Sprintf("<%d> %s %s %s[%d]: %s",
		p, timestamp.Format(time.RFC3339), hostname, tag, os.Getpid(), content)
	return msg
}

// UnixFormatter omits the hostname, because it is only used locally.
func UnixFormatter(timestamp time.Time, p Priority, _, tag, content string) string {
	msg := fmt.Sprintf("<%d>%s %s[%d]: %s",
		p, timestamp.Format(time.Stamp), tag, os.Getpid(), content)
	return msg
}

// RFC3164Formatter provides an RFC 3164 compliant message.
func RFC3164Formatter(timestamp time.Time, p Priority, hostname, tag, content string) string {
	msg := fmt.Sprintf("<%d>%s %s %s[%d]: %s",
		p, timestamp.Format(time.Stamp), hostname, tag, os.Getpid(), content)
	return msg
}
...
func RFC5424Formatter(timestamp time.Time, p Priority, hostname, tag, content string) string {
	pid := os.Getpid()
	appName := truncateStartStr(os.Args[0], appNameMaxLength)
	msg := fmt.Sprintf("<%d>%d %s %s %s %d %s - %s",
		p, 1, timestamp.Format(time.RFC3339), hostname, appName, pid, tag, content)
	return msg
}
